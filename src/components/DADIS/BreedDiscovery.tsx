...
import { Panel, Stack, Button, Input, Badge, Spinner, Surface, IconButton, Card, DataTable, EmptyState, Skeleton } from '@nekazari/ui-kit';
...
const breedColumns = [
  ...
  { accessorKey: 'speciesId', header: 'Species ID', cell: (info: any) => React.createElement(Badge, { intent: 'info' }, String(info.getValue())) },
  { accessorKey: 'countryISO3', header: 'Country' },
  { accessorKey: 'transboundaryId', header: 'Transboundary', cell: (info: any) => info.getValue() || '—' },
];
            ...
            <Stack gap="stack">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-nkz-stack">
                {['GBIF Livestock', 'AGROVOC', 'EPPO', 'GlobalTreeSearch', 'EU Pesticides', 'CPVO Varieties'].map((name) => (
                  <Card key={name} padding="sm"><div className="flex items-start gap-2"><CheckCircle className="w-3.5 h-3.5 text-nkz-success flex-shrink-0 mt-0.5" /><div><p className="text-nkz-sm font-medium">{name}</p></div></div></Card>
                ))}
              </div>
            </Stack>
          </Panel.Body>
    ...
    <Panel>
      ...
      <Panel.Body>
        <Stack gap="stack">
          ...
          {refLoading ? <div className="flex items-center justify-center py-4"><Spinner size="sm" /></div> : (
            <div className="flex gap-3 items-end flex-wrap">
              <div className="min-w-[160px]"><label className="text-nkz-xs font-medium text-nkz-text-muted block mb-1">{t('dadis.filters.country')}</label><select className="w-full h-9 rounded-nkz-md border border-nkz-border bg-nkz-surface px-nkz-stack text-nkz-sm" value={selectedCountry} onChange={(e) => setSelectedCountry(e.target.value)}><option value="">{t('dadis.filters.allCountries')}</option>{countries.map((c) => <option key={c.iso3} value={c.iso3}>{c.name}</option>)}</select></div>
              <div className="min-w-[140px]"><label className="text-nkz-xs font-medium text-nkz-text-muted block mb-1">{t('dadis.filters.species')}</label><select className="w-full h-9 rounded-nkz-md border border-nkz-border bg-nkz-surface px-nkz-stack text-nkz-sm" value={selectedSpecies} onChange={(e) => setSelectedSpecies(e.target.value)}><option value="">{t('dadis.filters.allSpecies')}</option>{species.map((s) => <option key={s.id} value={String(s.id)}>{s.name || s.id}</option>)}</select></div>
              <div className="min-w-[160px]"><label className="text-nkz-xs font-medium text-nkz-text-muted block mb-1">{t('dadis.filters.classification')}</label><select className="w-full h-9 rounded-nkz-md border border-nkz-border bg-nkz-surface px-nkz-stack text-nkz-sm" value={classification} onChange={(e) => setClassification(e.target.value)}><option value="all">{t('dadis.filters.all')}</option><option value="local">{t('dadis.filters.local')}</option><option value="transboundary">{t('dadis.filters.transboundary')}</option></select></div>
          ...
          {loading ? <div className="flex items-center justify-center py-nkz-section"><Spinner size="md" /></div>
            : breeds.length > 0 ? <DataTable columns={breedColumns} data={breeds} />
            : !error ? <EmptyState icon={<Database className="w-8 h-8 text-nkz-text-muted" />} title={t('dadis.empty.title')} description={t('dadis.empty.hint')} /> : null}
        </Stack>
      </Panel.Body>
    </Panel>
  );
