  ...
  const handleRun = async () => {
    ...
    es.onmessage = (event) => {
      try {
        ...
        // Ignore non-JSON heartbeat/comment lines
      }
    };
    es.onerror = () => {
      es.close();
    ...
    try {
      ...
      if (sources.trim()) {
        body.sources = sources.split(',').map((s: string) => s.trim());
      }
            ...
            <Stack gap="stack">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <div className="text-center p-nkz-inline bg-nkz-surface rounded-nkz-md">
                  <span className="block text-nkz-lg font-bold text-nkz-accent-base">
                    {result.entities_before_dedup}
                  </span>
                  <span className="text-nkz-xs text-nkz-text-muted">
                    {t('pipeline.result.entitiesRaw')}
                  </span>
                </div>
                <div className="text-center p-nkz-inline bg-nkz-surface rounded-nkz-md">
                  <span className="block text-nkz-lg font-bold text-nkz-accent-base">
                    {result.entities_after_dedup}
                  </span>
                  <span className="text-nkz-xs text-nkz-text-muted">
                    {t('pipeline.result.entitiesDedup')}
                  </span>
                </div>
                <div className="text-center p-nkz-inline bg-nkz-surface rounded-nkz-md">
                  <span className="block text-nkz-lg font-bold text-nkz-accent-base">
                  ...
                  <span className="text-nkz-xs text-nkz-text-muted">
                    {t('pipeline.result.relationships')}
                  </span>
                </div>
                <div className="text-center p-nkz-inline bg-nkz-surface rounded-nkz-md">
                  <span className="block text-nkz-lg font-bold text-nkz-accent-base">
                    {result.crossref_matches}
                  </span>
                  <span className="text-nkz-xs text-nkz-text-muted">
                    {t('pipeline.result.crossrefs')}
                  </span>
                </div>
                <div className="text-center p-nkz-inline bg-nkz-surface rounded-nkz-md">
                  <span className="block text-nkz-lg font-bold text-nkz-accent-base">
              ...
              {result.errors.length > 0 && (
                <div className="bg-nkz-danger-soft rounded-nkz-md p-nkz-inline border border-nkz-danger">
                  <h4 className="text-nkz-xs font-semibold text-nkz-danger-strong mb-1">
                    {t('pipeline.result.errorsTitle')}
                  </h4>
                ...
                {history.map((h, i) => (
                  <tr key={i} className="border-t border-nkz-border">
                    ...
                    <td className="py-1 pr-2">
                      {h.success
                        ? <CheckCircle className="w-3.5 h-3.5 text-nkz-success inline" />
                        : <XCircle className="w-3.5 h-3.5 text-nkz-danger inline" />
                      }
                      {h.errors > 0 && (
                        <span className="text-nkz-danger ml-1">{h.errors} err</span>
                      )}
                    </td>
