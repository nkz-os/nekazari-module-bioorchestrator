/**
 * CropManagementView — the shared core of crop management.
 *
 * Renders the full crop-management surface (crop context, assign/change crop,
 * soil, vegetation, rotation, scenario simulator) given a `parcelId`.
 *
 * Two thin wrappers consume this component:
 *   - RecommendationsPanel  (slot in the unified viewer — wraps in SlotShell)
 *   - CropManagement        (standalone page tool in the module's own page)
 *
 * This keeps a SINGLE source of truth for the logic while allowing isolated
 * development/testing on the module page before extending to the viewer.
 */
...
export interface CropManagementViewProps {
  parcelId: string;
  parcelName?: string;
  lat?: number | null;
  lon?: number | null;
...
const CropManagementView: React.FC<CropManagementViewProps> = ({ parcelId, parcelName, lat, lon }) => {
  ...
  if (cropContextLoading) {
    return <Stack gap="stack"><Skeleton variant="rect" height="60px" /></Stack>;
  }
...
export default CropManagementView;
