...
const PhenologyContribute: React.FC<Props> = ({ onClose }) => {
  ...
  const api = useBioApi();
  const [species, setSpecies] = useState(''); const [stage, setStage] = useState('');
  const [cultivar, setCultivar] = useState(''); const [kc, setKc] = useState('');
  const [d1, setD1] = useState(''); const [d2, setD2] = useState('');
  const [mdsRef, setMdsRef] = useState(''); const [doi, setDoi] = useState('');
  const [author, setAuthor] = useState(''); const [conditions, setConditions] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mdsRef) params.set('mds_ref', mdsRef); if (cultivar) params.set('cultivar', cultivar);
      if (doi) params.set('doi', doi); if (author) params.set('author', author);
      if (conditions) params.set('conditions', conditions); if (email) params.set('contact_email', email);
      await api.contributePhenology(params);
      setResult('success'); setTimeout(onClose, 2000);
    } catch (e: any) { setResult(e.message || 'Unknown error'); }
