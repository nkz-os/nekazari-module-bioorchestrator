...
import { Stack, Card, Badge, Button, DetailGrid, DetailItem, Skeleton, Tabs } from '@nekazari/ui-kit';
  ...
  return (
    <Stack gap="md">
      ...
      <Stack direction="row" gap="sm">
        {tabs.map(tab => (
          <Badge key={tab.id} variant={tab.available ? 'success' : 'muted'}>
            {tab.label} {tab.available ? '✓' : '✗'}
          </Badge>
        ))}
      </Stack>

      <Tabs active={activeTab} onChange={setActiveTab} tabs={tabs.map(t => ({ id: t.id, label: t.label }))} />
      ...
      {activeTab === 'npk' && (
        !da.npk ? (
          <EmptyState icon={<Droplets />} title={t('catalog.detail.missing')} variant="warning" />
        ) : (
          <Card>
            <DetailGrid>
      ...
      <Stack direction="row" gap="sm">
        <Button variant="outline" onClick={onContribute} icon={<PlusCircle size={14} />}>
          {t('catalog.detail.contributeData')}
        </Button>
        <Button variant="outline" onClick={onViewInParcel} icon={<Link size={14} />}>
