"""IFAPA → canonical node transformation (BaseIngester subclass).

Reads the IFAPA JSON-LD file and transforms it into canonical node
dicts with mergeKeys and registry enrichment.

Usage:
    python -m app.ingestion.ifapa_ingester \
        --jsonld ../nkz-ifapa-scraper/data/trials.jsonld
"""
...
class IfapaIngester(BaseIngester):
    """Transform IFAPA JSON-LD to canonical node dicts."""
...
async def main():
    parser = argparse.ArgumentParser(description="IFAPA → transform nodes")
