"""LfL Bayern → canonical node transformation (BaseIngester subclass).

Reads the LfL Bayern JSON-LD file and transforms it into canonical node
dicts with mergeKeys and registry enrichment.

Usage:
    python -m app.ingestion.lfl_ingester \
        --jsonld ../nkz-lfl-scraper/data/trials.jsonld
"""
...
class LflIngester(BaseIngester):
    """Transform LfL Bayern JSON-LD to canonical node dicts."""

    SOURCE_ID = "LFL-BAYERN"
...
async def main():
    parser = argparse.ArgumentParser(description="LfL Bayern → transform nodes")
