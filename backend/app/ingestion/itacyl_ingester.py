"""ITACyL → canonical node transformation (BaseIngester subclass).

Reads the ITACyL JSON-LD file and transforms it into canonical node
dicts with mergeKeys and registry enrichment.

Usage:
    python -m app.ingestion.itacyl_ingester \
        --jsonld ../nkz-itacyl-scraper/data/trials.jsonld
"""
...
class ItacylIngester(BaseIngester):
    """Transform ITACyL JSON-LD to canonical node dicts."""
...
async def main():
    parser = argparse.ArgumentParser(description="ITACyL → transform nodes")
